"""Figure for Section 6.3 — HZIED baseline applied to PLATO P4 and P5.

Parameters match the Schlecker+2024 fiducial (Turbet+2020 steam model,
Bergsten+2022 occurrence rates, x_H2O=0.005, f_rgh=0.8) with age_mode='sampled'
to draw stellar ages from the Gaia/FLAME age distribution of each PLATO sample.
P1 and P2 are excluded: the depth+mass filter leaves fewer than one window
of detectable rocky planets around their large FGK host stars.
"""
...
def main() -> None:
    ...
    for ax, res, label in zip(axes, [res_p4, res_p5], ['P4', 'P5']):
        ...
        title = (
            f'{label}  Turbet, '
            r'$M\in[0.1,\,2.05]\,M_\oplus$, depth $\geq80\,$ppm'
            '\n'
    ...
    axes[1].set_ylabel('')
    fig.tight_layout()
    save_figure(fig, "plato_baseline_hzied")
